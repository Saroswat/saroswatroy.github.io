...
export function CertificationExplorer() {
  ...
  return <>
    ...
    <div className="cert-grid">{filtered.map(item => <article className="cert-card" key={`${item.provider}-${item.title}`}><p className="eyebrow">{item.category}</p><h2>{item.title}</h2><p>{item.provider}</p></article>)}</div>
  </>;
}
