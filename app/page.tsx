...
export default function Home() {
  ...
  return <>
    ...
    <section className="section"><SectionHeading eyebrow="Recent certifications" title="Continuous learning, deliberately applied." intro="The latest additions to a complete professional learning record across AI, data science, cybersecurity, product management and technology operations." /><div className="cert-strip">{certifications.slice(0, 4).map(item => <article key={item.title}><span>{item.provider} · {item.issued}</span><h3>{item.title}</h3></article>)}</div><Link className="section-link" href="/certifications">Browse all {certifications.length} credentials <span aria-hidden="true">→</span></Link></section>
